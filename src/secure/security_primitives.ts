...
export function calculateMessageAuthenticationCodeCBC(
  ...
  const blocks = Buffer.from(Buffer.concat([
    block0,
...
export function decryptCtr(
  ...
  // Decrypt the payload
  const decryptedData = Buffer.from(Buffer.concat([
...
export function encryptDataCtr(
  ...
  // Encrypt the payload
  const encryptedData = Buffer.from(Buffer.concat([
    cipher.update(payload),
    cipher.final()
  ]));
 ...
 */
export function deriveDeviceAuthenticationPassword(deviceAuthenticationPassword: string): Buffer {
