  ...
  formatAPDU: (value: string) => {
    if (!value) {
      ...
      if (typeof value === 'string') {
        ...
        if (apdu_data > 255) Log.get().warn('DPT4: must supply an ASCII character');
        return Buffer.from([apdu_data]);
      } else Log.get().warn('DPT4: Must supply a character or string');
    }
  },
  fromBuffer: (buf: Buffer) => {
    if (buf.length != 1) {
      Log.get().warn('DPT4: Buffer should be 1 byte long, got', buf.length);
      return null;
    } else {
