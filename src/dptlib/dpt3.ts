...
const config: DatapointConfig = {
  ...
  formatAPDU: (value: { decr_incr: number, data: number }) => {
    if (!value) Log.get().warn('DPT3: cannot write null value');
    else {
      const apdu_data = Buffer.alloc(1);
      if (typeof value === 'object' &&
        hasProp(value, 'decr_incr') &&
        hasProp(value, 'data')) {
        apdu_data[0] = (value.decr_incr ? 1 : 0) << 3 | (value.data & 0b00000111);
      } else {
  ...
  fromBuffer: (buf: Buffer) => {
    if (buf.length != 1) {
      Log.get().error('DPT3: Buffer should be 1 byte long, got', buf.length);
      return null;
    } else {
      return {
