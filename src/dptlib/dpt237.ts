...
const config: DatapointConfig = {
  ...
  formatAPDU: (value) => {
    if (!value) {
      ...
      if (
        ...
        Log.get().error("DPT237: Must supply an valid payload. See the WIKI.");
      }
      ...
      const MSB =
        "00000" +
        (value.convertorError === false ? "0" : "1") +
  ...
  fromBuffer: (buf) => {
    if (buf.length != 2) {
      Log.get().error("DPT237: Buffer should be 2 byte long, got", buf.length);
      return null;
    }
