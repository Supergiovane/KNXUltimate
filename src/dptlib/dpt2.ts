...
const config: DatapointConfig = {
  ...
  formatAPDU: (value: { priority: boolean, data: boolean }) => {
    if (!value) {
      ...
      if (typeof value === 'object' &&
        ...
        Log.get().error('DPT2: Must supply a value {priority:<bool>, data:<bool>}');
      }
  ...
  fromBuffer: (buf: Buffer) => {
    if (buf.length != 1) {
      Log.get().error('DPT2: Buffer should be 1 byte long, got', buf.length);
      return null;
    } else {
      return {
        priority: (buf[0] & 0b00000011) >> 1 ? true : false,
        data: (buf[0] & 0b00000001) ? true : false
      };
    }
  },
