/**
 * Provides KNX Data Point Type 213 encoding and decoding helpers.
 *
...
const config: DatapointConfig = {
	...
	formatAPDU(value) {
		...
		logger.error(
			'Must supply a payload like, for example: {Comfort:21, Standby:20, Economy:14, BuildingProtection:8}',
		)
		return null
	},
	...
	fromBuffer(buf) {
		if (buf.length !== 8) {
			logger.warn(
				'fromBuffer: buf should be 4x2 bytes long (got %d bytes)',
				buf.length,
			)
			return null
		}
