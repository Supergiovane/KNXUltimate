...
export default class KNXClient extends EventEmitter {
        ...
        () => {
          try {
            ...
            if (this._options.localSocketAddress === undefined) {
              this._options.localSocketAddress = (this._clientSocket as UFPSocket).address().address;
            }
          } catch (error) {
            if (this.sysLogger !== undefined && this.sysLogger !== null) {
              this.sysLogger.error(
    ...
    try {
      if (this.sysLogger !== undefined && this.sysLogger !== null) {
        this.sysLogger.trace(
          "isSixBits:" +
            isSixBits +
            " Includes (should be = isSixBits):" +
            [1, 2, 3, 5, 9, 10, 11, 14, 18].includes(iDatapointType) +
            " ADPU BitLenght:" +
            adpu.bitlength
        );
      }
  ...
  send(knxPacket: any): void {
    if (this.sysLogger !== undefined && this.sysLogger !== null) {
      try {
        if (
          ...
          knxPacket.constructor.name.toLowerCase() === "knxconnectrequest"
        ) {
        ...
        if (
          knxPacket.constructor.name !== undefined &&
          (knxPacket.constructor.name.toLowerCase() === "knxtunnelingrequest" ||
            knxPacket.constructor.name.toLowerCase() === "knxroutingindication")
        ) {
          let sTPCI = "";
          ...
          try {
            sDebugString =
              "Data: " + JSON.stringify(knxPacket.cEMIMessage.npdu);
            sDebugString +=
              " srcAddress: " + knxPacket.cEMIMessage.srcAddress.toString();
            sDebugString +=
              " dstAddress: " + knxPacket.cEMIMessage.dstAddress.toString();
          } catch (error) {}
          ...
          this.sysLogger.debug(
            "Sending KNX packet: " +
              ...
              " Dest:" +
              knxPacket.cEMIMessage.dstAddress.toString(),
            " Data:" +
              knxPacket.cEMIMessage.npdu.dataValue.toString("hex") +
              " TPCI:" +
              sTPCI
          );
        }
    ...
    if (
        ...
        (this._clientSocket as UFPSocket).send(
          ...
          (err) => {
            if (err) {
              if (this.sysLogger !== undefined && this.sysLogger !== null) {
                this.sysLogger.error(
                  "Sending KNX packet: Send UDP sending error: " +
                    err.message || "Undef error"
                );
              }
              try {
                this.emit(KNXClientEvents.error, err);
      ...
      try {
        (this._clientSocket as TCPSocket).write(knxPacket.toBuffer(), (err) => {
          if (err) {
            if (this.sysLogger !== undefined && this.sysLogger !== null) {
              this.sysLogger.error(
  ...
  write(dstAddress: KNXAddress | string, data: any, dptid: any): void {
    ...
    if (this._options.hostProtocol === "Multicast") {
      ...
      const knxPacketRequest = KNXProtocol.newKNXRoutingIndication(cEMIMessage);
  ...
  getConnectionStatus() {
    ...
    this._heartbeatTimer = setTimeout(() => {
      ...
      try {
        if (this.sysLogger !== undefined && this.sysLogger !== null)
          this.sysLogger.error(
            "KNXClient: getConnectionStatus Timeout " +
              this._heartbeatFailures +
              " out of " +
              this.max_HeartbeatFailures
          );
        // this.emit(KNXClientEvents.error, timeoutError);
      } catch (error) {}
  ...
  Disconnect() {
    ...
    if (this._channelID === null) {
      ...
      try {
        ...
        if (this.sysLogger !== undefined && this.sysLogger !== null)
          this.sysLogger.debug(
            "KNXClient: into Disconnect(), this._clientSocket.close(): " +
              this._options.ipAddr +
              ":" +
  ...
  _setDisconnected(_sReason = "") {
    try {
      if (this.sysLogger !== undefined && this.sysLogger !== null)
        this.sysLogger.debug(
          "KNXClient: called _setDisconnected " +
            this._options.ipAddr +
            ":" +
            this._options.ipPort +
            " " +
  ...
  _setTimerWaitingForACK(knxTunnelingRequest) {
    ...
    const timeoutErr = new errors.RequestTimeoutError(
      `seqCounter:${knxTunnelingRequest.seqCounter}, DestAddr:${
        knxTunnelingRequest.cEMIMessage.dstAddress.toString() || "Non definito"
      },  AckRequested:${
          ...
          if (this.sysLogger !== undefined && this.sysLogger !== null)
            this.sysLogger.error(
              "KNXClient: _setTimerWaitingForACK: " +
                ...
                " no ACK received. ABORT sending datagram with seqNumber " +
                this._getSeqNumber() +
                " from " +
    ...
    try {
      ...
      try {
        if (this.sysLogger !== undefined && this.sysLogger !== null) {
          ...
          this.sysLogger.trace(
            "Received KNX packet: _processInboundMessage, " +
              sProcessInboundLog +
              " ChannelID:" +
              this._channelID ||
              "??" +
      ...
      if (knxHeader.service_type === KNX_CONSTANTS.ROUTING_LOST_MESSAGE) {
        ...
        return;
      } else if (knxHeader.service_type === KNX_CONSTANTS.ROUTING_BUSY) {
        try {
      ...
      if (knxHeader.service_type === KNX_CONSTANTS.SEARCH_RESPONSE) {
        ...
        if (this._connectionState === STATE.CONNECTING) {
          ...
          try {
            if (this.sysLogger !== undefined && this.sysLogger !== null)
              this.sysLogger.debug(
                "Received KNX packet: CONNECT_RESPONSE, ChannelID:" +
                  this._channelID +
                  " Host:" +
                  this._options.ipAddr +
                  ":" +
        ...
        try {
          if (this.sysLogger !== undefined && this.sysLogger !== null)
            this.sysLogger.debug(
              "Received KNX packet: DISCONNECT_RESPONSE, ChannelID:" +
                this._channelID +
                " Host:" +
                this._options.ipAddr +
                ":" +
        ...
        try {
          if (this.sysLogger !== undefined && this.sysLogger !== null)
            this.sysLogger.error(
              "Received KNX packet: DISCONNECT_REQUEST, ChannelID:" +
                this._channelID +
                " Host:" +
                this._options.ipAddr +
                ":" +
        ...
        if (knxTunnelingRequest.channelID !== this._channelID) {
          try {
            this.sysLogger.debug(
              "Received KNX packet: TUNNELING: L_DATA_IND, NOT FOR ME: MyChannelID:" +
                this._channelID +
                " ReceivedPacketChannelID: " +
                knxTunnelingRequest.channelID +
                " ReceivedPacketseqCounter:" +
        ...
        try {
          ...
          this.sysLogger.error(
            "Received KNX packet: TUNNELING: L_DATA_IND, ERROR BUOLDING THE TUNNELINK ACK: " +
              error.message +
              " MyChannelID:" +
              this._channelID +
              " ReceivedPacketChannelID: " +
        ...
        if (
          knxTunnelingRequest.cEMIMessage.msgCode === CEMIConstants.L_DATA_IND
        ) {
          // Composing debug string
          try {
            if (this.sysLogger !== undefined && this.sysLogger !== null) {
              ...
              try {
                sDebugString =
                  "Data: " +
                  JSON.stringify(knxTunnelingRequest.cEMIMessage.npdu);
                sDebugString +=
                  " srcAddress: " +
                  knxTunnelingRequest.cEMIMessage.srcAddress.toString();
                sDebugString +=
              ...
              this.sysLogger.debug(
                "Received KNX packet: TUNNELING: L_DATA_IND, " +
                  sDebugString +
                  " ChannelID:" +
                  this._channelID +
                  " seqCounter:" +
          ...
          knxTunnelingRequest.cEMIMessage.msgCode === CEMIConstants.L_DATA_CON
        ) {
          try {
            if (this.sysLogger !== undefined && this.sysLogger !== null)
              this.sysLogger.debug(
                "Received KNX packet: TUNNELING: L_DATA_CON, ChannelID:" +
                  this._channelID +
                  " seqCounter:" +
                  knxTunnelingRequest.seqCounter +
                  " Host:" +
        ...
        try {
          if (this.sysLogger !== undefined && this.sysLogger !== null)
            this.sysLogger.debug(
              "Received KNX packet: TUNNELING: TUNNELING_ACK, ChannelID:" +
                this._channelID +
                " seqCounter:" +
        ...
        if (!this._options.suppress_ack_ldatareq) {
          if (knxTunnelingAck.seqCounter === this._getSeqNumber()) {
            ...
            try {
              if (this.sysLogger !== undefined && this.sysLogger !== null)
                this.sysLogger.debug(
                  "Received KNX packet: TUNNELING: DELETED_TUNNELING_ACK FROM PENDING ACK's, ChannelID:" +
                    this._channelID +
                    " seqCounter:" +
                    knxTunnelingAck.seqCounter +
                    " Host:" +
            ...
            try {
              if (this.sysLogger !== undefined && this.sysLogger !== null)
                this.sysLogger.error(
                  "Received KNX packet: TUNNELING: Unexpected Tunnel Ack with seqCounter = " +
                    knxTunnelingAck.seqCounter
                );
            } catch (error) {}
      ...
      } else if (knxHeader.service_type === KNX_CONSTANTS.ROUTING_INDICATION) {
        // 07/12/2021 Multicast routing indication
        const knxRoutingInd = knxMessage;
        if (knxRoutingInd.cEMIMessage.msgCode === CEMIConstants.L_DATA_IND) {
          // Composing debug string
          try {
            if (this.sysLogger !== undefined && this.sysLogger !== null) {
              let sDebugString = "???";
              try {
                sDebugString =
                  "Data: " + JSON.stringify(knxRoutingInd.cEMIMessage.npdu);
                sDebugString +=
                  " srcAddress: " +
                  knxRoutingInd.cEMIMessage.srcAddress.toString();
                sDebugString +=
                  " dstAddress: " +
                  knxRoutingInd.cEMIMessage.dstAddress.toString();
              } catch (error) {}
              this.sysLogger.debug(
                "Received KNX packet: ROUTING: L_DATA_IND, " +
                  sDebugString +
                  " Host:" +
                  this._options.ipAddr +
                  ":" +
          ...
          knxRoutingInd.cEMIMessage.msgCode === CEMIConstants.L_DATA_CON
        ) {
          try {
            if (this.sysLogger !== undefined && this.sysLogger !== null)
              this.sysLogger.debug(
                "Received KNX packet: ROUTING: L_DATA_CON, Host:" +
                  this._options.ipAddr +
                  ":" +
                  this._options.ipPort
              );
        ...
        if (knxHeader.service_type === this._awaitingResponseType) {
          if (
            ...
            try {
              if (this.sysLogger !== undefined && this.sysLogger !== null)
                this.sysLogger.debug(
                  "Received KNX packet: CONNECTIONSTATE_RESPONSE, ChannelID:" +
                    this._channelID +
                    " Host:" +
                    this._options.ipAddr +
                    ":" +
            ...
            if (
              knxConnectionStateResponse.status !== KNX_CONSTANTS.E_NO_ERROR
            ) {
              try {
                this.emit(
                  KNXClientEvents.error,
              ...
              this._setDisconnected(
                "Awaiting response " +
                  this._awaitingResponseType +
                  ", received connection state response  with status " +
                  knxConnectionStateResponse.status
              );
            } else {
              if (this._heartbeatTimer !== null)
      ...
      try {
        if (this.sysLogger !== undefined && this.sysLogger !== null)
          this.sysLogger.error(
            "Received KNX packet: Error processing inbound message: " +
              e.message +
              " " +
              sProcessInboundLog +
              " ChannelID:" +
              ...
              ":" +
              this._options.ipPort +
              ". This means that KNX-Ultimate received a malformed Header or CEMI message from your KNX Gateway."
          );
      } catch (error) {}
      try {
