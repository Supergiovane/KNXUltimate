/**
 * Example demonstrating KNXClient.writeRaw().
 *
...
async function main() {
	...
	client.on(KNXClientEvents.connected, async () => {
		console.log('Connected. WARNING: this example will WRITE on your KNX bus.')
		...
		client.writeRaw('1/1/1', Buffer.from([0x01]), 1) // ON
		await wait(200)

		// Example 2: build the APDU using dptlib, then send it as raw bytes.
		// This is useful when you already have a Buffer (or you want to control bitlength explicitly).
		// Replace the GA with a real one from your ETS project.
		const temperature = encodeWithDptLib(21.5, '9.001') // 2-byte float (°C)
		client.writeRaw('1/1/2', temperature.data, temperature.bitlength)
		await wait(200)
